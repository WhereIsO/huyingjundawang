// BackupTool core module notes: encoding.cpp
...
std::string wideToUtf8(const std::wstring& value) {
    ...
    if (needed <= 0) {
        throw BackupError(ErrorCode::Unknown, "宽字符转 UTF-8 失败");
    }
...
std::wstring utf8ToWide(const std::string& value) {
    ...
    const int needed = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
                                           ...
                                           nullptr, 0);
    if (needed <= 0) {
        throw BackupError(ErrorCode::Unknown, "UTF-8 转宽字符失败");
    }
...
std::string normalizeRelPath(const std::filesystem::path& path) {
    std::string rel = pathToUtf8(path.lexically_normal());
...
std::wstring longPath(const std::filesystem::path& path) {
    ...
    if (value.rfind(LR"(\\?\)", 0) == 0 || value.rfind(LR"(\\.\)", 0) == 0) {
        return value;
    }
