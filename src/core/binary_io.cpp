// BackupTool core module notes: binary_io.cpp
...
void ByteWriter::zstring(const std::string& value) {
...
void ByteReader::ensure(std::size_t size) const {
    if (offset_ + size > data_.size()) {
        throw BackupError(ErrorCode::PkgCorrupted, "备份包字段长度越界");
    }
}
